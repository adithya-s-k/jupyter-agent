...
def render_instruction(question: str, files_used: list[str], packages_used: list[str]) -> str:
    ...
    footer = (
        "\n\n---\n"
        "**Work it out step by step.** Inspect the data first (head, shape, "
        "dtypes), write down what you observe, plan the computation, then "
        "execute it. If your agent has a notes/scratchpad tool, USE IT — "
        "jot down intermediate results, the columns you found, and the "
        "exact formula you're applying before the final calc. This is "
        "more reliable than reasoning silently across many tool calls.\n\n"
        "**Submission protocol (READ CAREFULLY):**\n"
        "1. Compute the answer in your sandbox.\n"
        "2. Write **only the answer value** (no labels, no units, no "
        "trailing newline noise) to the absolute path `/workdir/answer.txt`. "
        "Examples of the right shape (replace `<value>` with your computed "
        "answer — do NOT write the literal string `<value>`):\n"
        "   - Shell: `echo -n \"<value>\" > /workdir/answer.txt`\n"
        "   - Python: `open(\"/workdir/answer.txt\",\"w\").write(str(<value>))`\n"
        "3. **Do NOT use patch-style tools** (`apply_patch`, `edit`, diff "
        "patches) to write `answer.txt` — they resolve paths relative to a "
        "workspace root which may not include `/workdir/`. Always use a "
        "direct-write tool (shell redirect, file write, or your custom "
        "`shell_write_content_to_file` if available) with the **absolute** "
        "path `/workdir/answer.txt`.\n"
        "4. After the file is written, stop calling tools.\n\n"
        "The grader does exact match → numeric tolerance → LLM judge against "
        "the gold answer. Keep the answer short and concise."
    )
    return body + footer
