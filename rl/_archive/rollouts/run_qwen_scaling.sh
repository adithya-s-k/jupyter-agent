#!/usr/bin/env bash
# Qwen scaling matrix on the easy tesla task — 3 Qwen sizes × 4 agents = 12 runs.
# Validates that the same task + same agent contract works across model sizes
# via HF Inference Providers (nscale).
#
# Models (smaller → bigger, all on nscale via HF router):
#   Qwen3-4B-Instruct-2507     — student-size reference
#   Qwen3-8B
#   Qwen3-14B
#
# Agents:
#   jupyter-tool   (custom, 4-tool stateful kernel)
#   mini-swe-agent (Harbor built-in, bash only)
#   seta-tool      (custom, 10-tool SETA-style with persistent notes)
#   opencode       (Harbor built-in, bash/read/write/edit)
#
# Usage: bash rl/rollouts/run_qwen_scaling.sh
...
# (size_label, our-format-id, opencode-format-id)
declare -a MODELS=(
